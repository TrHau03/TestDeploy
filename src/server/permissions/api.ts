...
export async function updatePermission(id: number, body: PermissionUpdateRequest): Promise<Permission> {
  const response = await http.put(HTTPEndpointDynamic.UPDATE_PERMISSION(id), { json: body }).json<{ result: Permission }>();
  return response.result;
}

export async function deletePermission(id: number): Promise<{ message: string }> {
  const response = await http.delete(HTTPEndpointDynamic.DELETE_PERMISSION(id)).json<{ result: { message: string } }>();
  return response.result;
}
