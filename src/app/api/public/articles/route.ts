...
import { getAllArticles, createArticle } from "@/server/article/api";
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { _metadata: { success: false }, error: "Failed to fetch articles" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { _metadata: { success: false }, error: "Failed to create article" },
      { status: 400 }
    );
  }
}
